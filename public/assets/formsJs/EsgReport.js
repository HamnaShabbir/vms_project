...
function CallDatatable() {
    ...
    $("#result").DataTable({
        ...
        columns: [
            ...
            { data: 'framework_id', name: 'framework_id' },
            { data: 'reporting_convention_id', name: 'reporting_convention_id' },
            { data: 'material_topic_id', name: 'material_topic_id' },
            { data: 'auditor_id', name: 'auditor_id' },
...
$("#create_form").submit(function (e) {
    ...
    // Form validation
    var isValid = true;
    $("#create_form input[required], #create_form select[required]").each(function () {
...
$('body').delegate(".edit_row", 'click', function () {
    ...
    $.post(edit_url, {
        ...
        $("#edit_from_dt").val(res.from_date);
        $("#edit_to_dt").val(res.to_date);
        $("#edit_lob_id").val(res.lob_id);
        $("#edit_framework_id").val(res.framework_id);
        $("#edit_reporting_convention_id").val(res.reporting_convention_id);
        $("#edit_material_topic_id").val(res.material_topic_id);
        $("#edit_auditor_id").val(res.auditor_id);
...
$('body').delegate(".entity", 'change', function (e) {
    e.preventDefault();
    var id = $(this).val();
    var url = $('#result').data('edit_url');
    $.ajax({
        type:'POST',
        url: url,
        data: {'fetch':'GetLineOfBussiness','id':id},
        success: function (data) {
            $(".lob_id").empty();
            data.forEach(function(i) {
                var newOption = new Option(i.title, i.id);
                $(".lob_id").append(newOption);
            });
        },
        error: function (data) {
            toastr.error("Not Edit Successfully!");
