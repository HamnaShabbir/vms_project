...
function CallDatatable() {
    ...
    $("#result").DataTable({
        ...
        columns: [
            { data: 'id', name: 'id' },
            { data:'wt_id',name:'wt_id'},
            { data:'mu_id',name:'mu_id'},
            { data:'wd_from_disposal',name:'wd_from_disposal'},
            { data:'rm_id',name:'rm_id'},
            { data:'onsite_recovery',name:'onsite_recovery'},
            { data:'offsite_recovery',name:'offsite_recovery'},
            { data:'total',name:'total'},
            { data:'esg_report_id',name:'esg_report_id'},
...
$('body').delegate(".edit_row", 'click', function () {
    ...
    $.post(edit_url, {
        ...
        $('#id').val(res.id);
        $('#edit_wt_id').val(res.wt_id);
        $('#edit_wd_from_disposal').val(res.wd_from_disposal);
        $('#edit_onsite_recovery').val(res.onsite_recovery);
        $('#edit_offsite_recovery').val(res.offsite_recovery);
        $('#edit_total').val(res.total);
        $('#edit_mu_id').val(res.mu_id);
        $('#edit_rm_id').val(res.rm_id);
        $('#edit_esg_report_id').val(res.esg_report_id);
    })
    ...
    $.post(trashed_data_url, {
        ...
        $("#trashed_data_result").DataTable({
            ...
            columns: [
                { data: 'id', name: 'id' },
                {
                    data: 'wt_id', name: 'wt_id', render: function (data, full, row, met) {
                    return row.get_waste_type.title;
                    }
                },
                {
                    data: 'mu_id', name: 'mu_id', render: function (data, full, row, met) {
                    return row.get_measure_units.title;
                    }
                },
                {
                    data: 'rm_id', name: 'rm_id', render: function (data, full, row, met) {
                    return row.get_recovery_method.title;
                    }
                },
                { data:'wd_from_disposal',name:'wd_from_disposal'},
                { data:'onsite_recovery',name:'onsite_recovery'},
                ...
                { data:'total',name:'total'},
                {
                    data: 'esg_report_id', name: 'esg_report_id', render: function (data, full, row, met) {
                    return row.get_esg_report.title;
                    }
                },             
                { data: 'created_at', name: 'created_at' },
                { data: 'updated_at', name: 'updated_at' },
