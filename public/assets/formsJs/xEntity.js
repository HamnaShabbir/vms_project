...
function CallDatatable() {
    ...
    $("#result").DataTable({
        ...
        columns: [
            ...
            { data: 'parent_entity_id', name: 'parent_entity_id' },
...
$('body').delegate(".edit_row", 'click', function () {
    ...
    $.post(edit_url, {
        ...
        $('#edit_parent_entity_id').val(res.parent_entity_id)
        $('#edit_status').val(res.status)
        $('#edit_vission').text(res.vision)
        $('#edit_mission').val(res.mission)
        $('#edit_values').val(res.values)
    })
    ...
    $.post(trashed_data_url, {
        ...
        $("#trashed_data_result").DataTable({
            ...
            columns: [
                ...
                { data: 'title', name: 'title' },
                {
                    data: 'parent_entity_id', name: 'parent_entity_id', render: function (data, full, row, met) {
                         return row.GetEntity ? row.GetEntity.title : 'Parent Entity';
                    }
                },
                { data: 'status', name: 'status' },
                { data: 'vision', name: 'vision' },
